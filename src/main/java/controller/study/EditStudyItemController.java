...
public class EditStudyItemController implements Controller {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		StudyGroup study = manager.findStudyById(context.ownerId);
		if (!AuthorizationUtils.canManageStudy(accessManager, memberId, context.ownerId)) {
			return "redirect:/mystudy/view?groupId=" + context.ownerId + "&selectedDate=" + selectedDate;
		}

		if ("GET".equalsIgnoreCase(request.getMethod())) {
			fillFormAttributes(request, context, study, selectedDate);
		...
		try {
			...
			MultipartUploadUtils.saveAttachments(request, "study", context.itemType, id);
			request.getSession().setAttribute("flashMessage", "항목이 수정되었습니다.");
			return "redirect:/study/itemDetail?type=" + context.itemType + "&id=" + id + "&selectedDate="
					+ selectedDate;
		} catch (IllegalArgumentException ex) {
			request.getSession().setAttribute("flashError", ex.getMessage());
			return "redirect:/study/editItem?type=" + context.itemType + "&id=" + id + "&selectedDate="
					+ selectedDate;
		} catch (Exception ex) {
	...
	private void updateAssignment(HttpServletRequest request, StudyManager manager, Assignment original) {
		...
		assignment.setStudyId(original.getStudyId());
	...
	private void updateSchedule(HttpServletRequest request, StudyManager manager, Schedule original) throws Exception {
		Schedule schedule = new Schedule();
		schedule.setScheduleId(original.getScheduleId());
		schedule.setStudyGroupId(original.getStudyGroupId());
	...
	private void fillFormAttributes(HttpServletRequest request, ItemContext context, StudyGroup study,
			String selectedDate) {
		...
		request.setAttribute("formAction", request.getContextPath() + "/study/editItem");
		request.setAttribute("backUrl", request.getContextPath() + "/study/itemDetail?type=" + context.itemType
	...
	private ItemContext loadContext(StudyManager manager, String type, int id) {
		if ("assignment".equals(type)) {
			Assignment assignment = manager.findAssignmentById(id);
			if (assignment == null) {
				return null;
			}
			ItemContext context = baseContext(id, "assignment", "과제", assignment.getStudyId(), assignment.getTitle(),
		...
		if ("notice".equals(type)) {
			...
			return notice == null ? null : baseContext(id, "notice", "공지사항", notice.getStudyId(), notice.getTitle(),
		...
		ItemContext context = baseContext(id, "schedule", scheduleLabel(schedule), (int) schedule.getStudyGroupId(),
	...
	private String scheduleTitle(Schedule schedule) {
		...
		return "regular".equals(schedule.getType()) ? "정기모임" : scheduleLabel(schedule);
	}
