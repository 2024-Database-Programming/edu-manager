	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (request.getMethod().equals("GET")) {
			...
			if (updateId == null || updateId.isBlank()) {
				updateId = MemberSessionUtils.getLoginMemberId(session);
			}
			...
			request.setAttribute("curUserId", MemberSessionUtils.getLoginMemberId(session));
...
//                request.setAttribute("commList", commList);
		...
		if (!MemberSessionUtils.isLoginMember(updateId, session)
				&& !MemberSessionUtils.isLoginMember("admin", session)) {
			return "redirect:/member/view?id=" + updateId;
		}
