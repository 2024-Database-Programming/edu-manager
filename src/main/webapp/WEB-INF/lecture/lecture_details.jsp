...
<c:if test="${!isTeacher}">
			...
			<table class="study-location">
				...
				<tr class="icon">
					...
					<c:forEach var="schedule" items="${regularSchedules}">
						<td class="location-inform">${dayMap[schedule.dayOfWeek]}요일 ${schedule.startTime}~${schedule.endTime}</td>
					</c:forEach>
			...
			<div class="pinned-notice-box">
				<div class="team-header">
					...
					<a href="<c:url value='/lecture/listNotice'/>?groupId=${lectureInfo.lectureId}" class="more-link">더보기</a>
				</div>
						...
						<c:otherwise>
							<c:forEach var="notice" items="${noticeList}">
								<li class="member-item">
									<c:url var="noticeDetailUrl" value="/lecture/itemDetail">
										<c:param name="lectureId" value="${lectureInfo.lectureId}" />
			...
			<div class="pinned-assignment-box">
				<div class="team-header">
					...
					<a href="<c:url value='/lecture/listAssignment'><c:param name='groupId' value='${lectureInfo.lectureId}' /></c:url>" class="more-link">더보기</a>
				</div>
				<ul class="member-list">
					<c:choose>
						...
						<c:otherwise>
							<c:forEach var="assignment" items="${assignmentList}">
								<li class="member-item">
									<c:url var="assignmentDetailUrl" value="/lecture/itemDetail">
										<c:param name="lectureId" value="${lectureInfo.lectureId}" />
				...
				<div class="rectangle-1">
					<div class="classroom">
						...
						<div class="classroom_content">
							<ul>
								<c:forEach var="schedule" items="${regularSchedules}">
									<li class="detail-list-row">
										<c:url var="classDetailUrl" value="/lecture/itemDetail">
											<c:param name="lectureId" value="${lectureInfo.lectureId}" />
										...
										<a class="detail-item schedule-item" href="${classDetailUrl}">
											<span class="item-title">
												<c:choose>
													...
													<c:otherwise>정기 수업 - ${dayMap[schedule.dayOfWeek]}요일</c:otherwise>
												</c:choose>
											</span>
							...
							<c:if test="${isTeacher}">
								<a class="plus_button" href="<c:url value='/lecture/addSchedule'>
									<c:param name="lectureId" value="${lectureInfo.lectureId}" />
									<c:param name="selectedDate" value="${selectedDate}" />
							...
							<ul>
								<c:forEach var="schedule" items="${specialSchedules}">
									<c:if test="${schedule.type != 'class' && schedule.type != 'exam' && !fn:contains(schedule.title, '시험') && !fn:contains(schedule.title, '중간') && !fn:contains(schedule.title, '기말') && !fn:contains(schedule.title, '평가')}">
										<li class="detail-list-row">
											<c:url var="eventDetailUrl" value="/lecture/itemDetail">
												<c:param name="lectureId" value="${lectureInfo.lectureId}" />
												<c:param name="selectedDate" value="${selectedDate}" />
							...
							<ul>
								<c:forEach var="schedule" items="${specialSchedules}">
									<c:if test="${schedule.type == 'exam' || fn:contains(schedule.title, '시험') || fn:contains(schedule.title, '중간') || fn:contains(schedule.title, '기말') || fn:contains(schedule.title, '평가')}">
										<li class="detail-list-row">
											<c:url var="examDetailUrl" value="/lecture/itemDetail">
												<c:param name="lectureId" value="${lectureInfo.lectureId}" />
												<c:param name="selectedDate" value="${selectedDate}" />
						...
						<div class="assignment_content">
							<ul>
								<c:forEach var="assignment" items="${selectedAssignmentList}">
									<li class="detail-list-row">
										<c:url var="selectedAssignmentDetailUrl" value="/lecture/itemDetail">
											<c:param name="lectureId" value="${lectureInfo.lectureId}" />
											<c:param name="selectedDate" value="${selectedDate}" />
											<c:param name="type" value="assignment" />
											<c:param name="id" value="${assignment.id}" />
							...
							<c:if test="${isTeacher}">
								<a class="plus_button" href="<c:url value='/lecture/addAssignment'>
									<c:param name="lectureId" value="${lectureInfo.lectureId}" />
									<c:param name="selectedDate" value="${selectedDate}" />
								</c:url>">+</a>
							</c:if>
